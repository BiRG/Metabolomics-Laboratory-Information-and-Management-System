 ...
 * 
 */
public class CollectionResumableUpload {
	...
	private static final String[] USAGE_MESSAGE = {
		"Usage: java CollectionResumableUpload.jar --username <user> --password <pass>",
	};
	 ...
	 */
	CollectionResumableUpload(DocumentList docs, PrintStream out) {
		this.docs = docs;
		this.output = out;
	}
	...
	IOException, ServiceException, InterruptedException {
		...
		try {
			docs.login("pauleanderson.metabolomics", "birglab1");
		} catch (AuthenticationException e) {
			System.out.println("Invalid credentials. Check your username and password, and try again.");
			return;
		}		
		
		CollectionResumableUpload demo = new CollectionResumableUpload(
				docs, System.out);

		/* Prompt for username and password for omics_analysis website */
		String omics_analysis_name = "Paul Anderson";
		omics_analysis_name = omics_analysis_name.replaceAll(" ", "%20");
		String omics_analysis_password = "birglab";
		URL collectionUrl = new URL("http://birg.cs.wright.edu/omics_analysis/collections/8847.xml?name="+omics_analysis_name+
				"&password="+omics_analysis_password);
		BufferedReader in = new BufferedReader(new InputStreamReader(collectionUrl.openStream()));
		
		/* Prompt for collection description */
		System.out.println("\nPlease enter a description for this collection (e.g., Fatigue Analysis)");
		System.out.println("NOTE: Google collections may contain more than one omics_analysis collection");
		String collection_name = "New Test";
		
		DocumentListEntry folderEntry = demo.createFolder(collection_name);
		
		DocumentListEntry test = docs.getDocsListEntry("0AimNciFdCZS4dDVfUWJaRXlGbWNmMU1aRTRlUHFPclE");
		...
		while ((inputLine = in.readLine()) != null) {
			...
			if (fields[0].equals("Subject ID")) {
				...
				for (int i = 0; i < samples.size(); i++)       	    	
					((HashMap)samples.get(i)).put(fields[0], fields[i+1]);
			}
		}
		in.close();
		...
		List<String> files = Lists.newArrayList();
		for (int i = 0; i < 1; i++) {
			File temp = File.createTempFile(Long.toString(System.nanoTime()),".tab");
			files.add(temp.getAbsolutePath());
			//System.out.println(temp.getAbsolutePath());
			BufferedWriter writer = new BufferedWriter(new FileWriter(temp));
			HashMap sample = (HashMap)samples.get(i);
			Iterator iterator = sample.keySet().iterator();
			while(iterator. hasNext()){        
				Object key = iterator.next();
				writer.write(key+"\t"+sample.get(key));
			}
		}
		
		int chunkSize = DEFAULT_CHUNK_SIZE;
		Collection<DocumentListEntry> resDocs= demo.uploadFiles(DEFAULT_RESUMABLE_UPLOAD_URL, files, chunkSize);
		DocumentListEntry [] t = new DocumentListEntry[1];
		DocumentListEntry [] documents = resDocs.toArray(t);
	...
	public DocumentListEntry createFolder(String title) throws IOException, ServiceException {
		  ...
		  return docs.service.insert(feedUrl, newEntry);
	}
